...
export default async function AgentRevisionsPage() {
  ...
  const managerRevisions = await prisma.task.findMany({
    where: {
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Manager change requests show up here. Admin help requests live under
            the <span className="font-medium">Help</span> tab.
          </p>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Update the request and resubmit for review. For help from an assigned
          admin, use the <span className="font-medium">Help</span> tab.
        </p>
      </div>
